...
function makeDraggable(frame, handle, saveKey, lockedFn, onSaved)
    ...
    handle.InputBegan:Connect(function(inp)
        ...
        wasDrag = false  -- always reset so tap detection works even when UI is locked
        if lockedFn() then return end
        drag = true; ds = inp.Position; sp = frame.AbsolutePosition
    ...
    _UIS.InputChanged:Connect(function(inp)
        ...
        local vp = _getViewport()
        local size = frame.AbsoluteSize
        local nx = math.clamp(sp.X + d.X, 4, vp.X - size.X - 4)
        local ny = math.clamp(sp.Y + d.Y, 4, vp.Y - size.Y - 4)
...
function loadAndClampPosition(frame, key, defaultPos)
    ...
    -- Skip clamping if the frame has no size yet (hidden panel → AbsoluteSize == 0,0).
    task.defer(function()
        if frame and frame.Parent then
            if frame.AbsoluteSize.X > 0 and frame.AbsoluteSize.Y > 0 then
                clampToScreen(frame)
            else
                -- Panel is hidden; clamp once it becomes visible for the first time.
                local conn
...
local function _ltGetBestTarget(myHRP)
    -- sticky: keep locked target if still valid
    ...
    for _, p in ipairs(S.Players:GetPlayers()) do
        if p ~= S.LocalPlayer and _ltIsTargetValid(p.Character) then
...
function startLockTarget()
    ...
    lockTargetConnection = S.RunService.Heartbeat:Connect(function()
        ...
        if targetHRP and targetChar then
            ...
            local targetStandPos = dist3D > 0
                and (predictedPos - dirToTarget.Unit * _LT_STOP_DISTANCE)
                or predictedPos
            ...
            if distToStand > 1 then
                myH.AssemblyLinearVelocity = moveDir.Unit * spd
            else
                myH.AssemblyLinearVelocity = targetVel
            end
        else
            _ltLockedTarget = nil
...
function stopLockTarget()
    ...
    -- reset velocity & autorotate on stop
    pcall(function()
        local myC = character
