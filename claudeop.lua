...
_KEYBIND_SET={
    ...
    AUTO_STEAL_KEYBIND=true,DROP_BRAINROT_KEYBIND=true,DROP_V2_KEYBIND=true,FLOAT_KEYBIND=true,
...
function loadConfigKeys()
    ...
    for k, v in pairs(decoded) do
        ...
        elseif k == "STEAL_MODE" or k == "STEAL_NORMAL_RADIUS" or k == "STEAL_SEMI_RADIUS" or k == "STEAL_DURATION" then
...
dropV2PanelGui,dropV2PanelBtn=nil,nil
dropV2Running=false
dropV2Connection=nil
...
function runDropBrainrot()
    ...
    if CONFIG.AUTO_BAT_ENABLED then
        attacking=false
        pcall(function() if resetAutoBatMotion then resetAutoBatMotion() end end)
    end
    local char=LP.Character;if not char then return end
    local root=char:FindFirstChild("HumanoidRootPart");if not root then return end
    dropBrainrotActive=true
    if CONFIG.POST_DROP_HALT_ENABLED then
        if AutoPlayState and AutoPlayState.enabled then pcall(apStopLoop) end
        if AW and AW.enabled then pcall(awStopLoop) end
    end
    local t0=tick()
    local dc
...
function executeDropV2()
    if dropV2Running then return end
    local char=LP.Character
    local root=char and char:FindFirstChild("HumanoidRootPart")
    if not root then return end
    dropV2Running=true
    if dropV2Connection then
        dropV2Connection:Disconnect()
        dropV2Connection=nil
    end
    dropV2Connection=RunService.Heartbeat:Connect(function()
        if not dropV2Running or not root or not root.Parent then
            if dropV2Connection then dropV2Connection:Disconnect() end
            dropV2Connection=nil
            return
        end
...
function startLockTarget()
    ...
    if CONFIG.DROP_LOCK_ENABLED and not _dropLockBusy then
        if LP:GetAttribute("Stealing") then
            _dropLockBusy=true
            task.spawn(function()
                if CONFIG.DROP_V2_PANEL_VISIBLE then
                    executeDropV2()
                else
                    executeDrop()
                end
...
local BAT_V2_FOLLOW_DIST = 3.0
local BAT_V2_LOOK_AHEAD = 5.0
local BAT_V2_HEIGHT_OFFSET = 1.5
local BAT_V2_AIMBOT_SPEED = 60
local ROTATION_SPEED_MULTIPLIER = 20
local batV2CamConnection = nil
local function _lt2GetClosest()
    local char = LP.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
        ...
        if target and target.Character then
            ...
            if targetRoot then
                ...
                if targetHum and targetHum.MoveDirection.Magnitude > 0.1 then
                    moveDir = targetHum.MoveDirection.Unit
                end
                local offsetPos = targetRoot.Position + (moveDir * BAT_V2_FOLLOW_DIST) + Vector3.new(0, BAT_V2_HEIGHT_OFFSET, 0)
                local toTarget = offsetPos - root.Position
                local desiredVelocity = toTarget * 15
                if desiredVelocity.Magnitude > BAT_V2_AIMBOT_SPEED then
                    desiredVelocity = desiredVelocity.Unit * BAT_V2_AIMBOT_SPEED
                end
                root.AssemblyLinearVelocity = desiredVelocity
                local myPos = root.Position
                local lookTargetPos = targetRoot.Position + (moveDir * BAT_V2_LOOK_AHEAD)
                local toLook = lookTargetPos - myPos
                if toLook.Magnitude > 0.1 then
                    local goalCF = CFrame.lookAt(myPos, lookTargetPos)
                    ...
                    root.AssemblyAngularVelocity = root.CFrame:VectorToWorldSpace(
                        Vector3.new(rx * ROTATION_SPEED_MULTIPLIER, ry * ROTATION_SPEED_MULTIPLIER, rz * ROTATION_SPEED_MULTIPLIER)
                    )
                else
                    root.AssemblyAngularVelocity = Vector3.zero
                end
...
function startFloat()
    ...
    _FLT.conn=RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            local speed=CONFIG.FLOAT_SPEED or 50
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        local _dbV2OnTap
        dropV2PanelGui,dropV2PanelBtn,_dbV2OnTap=createFloatingPanel("DropV2","dropV2Panel","DROP_V2_PANEL_VISIBLE","DROP V2")
        _dbV2OnTap(function()
            task.spawn(executeDropV2)
        end)
        do
            local _carryOnTap
    ...
    local function createSettingsTab(parent)
        ...
        local floatingPanelsToggle=createToggle(uiContent,"Floating Panels",CONFIG.FLOATING_PANELS_ENABLED,function(state)
            ...
            local panels={
                ...
                speedGuiInstance,dropBrainrotPanelGui,dropV2PanelGui,tpDownPanelGui,
            ...
            if state then
                ...
                if CONFIG.DROP_V2_PANEL_VISIBLE and dropV2PanelGui then dropV2PanelGui.Enabled=true end
    ...
    local function createDuelTab(parent)
        ...
        local floatHeightInput=createNumberInput(mobContent,"Float Height",CONFIG.FLOAT_HEIGHT,0,9999,function(value)
            CONFIG.FLOAT_HEIGHT=value
        end)
        floatHeightInput.LayoutOrder=4
        local floatSpeedInput=createNumberInput(mobContent,"Float Speed",CONFIG.FLOAT_SPEED,1,99999,function(value)
            CONFIG.FLOAT_SPEED=value
        end)
        ...
        local dropV2PanelToggle,_dbV2PanelBtn=createPanelShowBtn(toolsContent,"Drop V2","DROP_V2_PANEL_VISIBLE",function()return dropV2PanelGui end,nil,function()
            dropV2Running=false
            if dropV2Connection then dropV2Connection:Disconnect();dropV2Connection=nil end
        end)
        dropV2PanelToggle.LayoutOrder=15
        table.insert(_duelToggleBtns,{btn=_dbV2PanelBtn,default=false})
        ...
        lockTargetPanelToggle.LayoutOrder=16
        ...
        resetBtn.MouseButton1Click:Connect(function()
            ...
            local defaults={
                LOCK_TARGET_PANEL_VISIBLE=false,DROP_BRAINROT_PANEL_VISIBLE=false,DROP_V2_PANEL_VISIBLE=false,
                ...
                POST_DROP_HALT_ENABLED=false,DROP_LOCK_ENABLED=false,CARRY_PANEL_VISIBLE=false,SPINBOT_SPEED=30,LOCK_TARGET_SPEED=55,
            ...
            for _,gui in ipairs({
                lockTargetPanelGui,dropBrainrotPanelGui,dropV2PanelGui,carryPanelGui,
    ...
    local function createGui()
        ...
        UIS.InputBegan:Connect(function(input,gpe)
            ...
            elseif isKeybindPressed(CONFIG.DROP_BRAINROT_KEYBIND,input)then
                executeDrop()
            elseif isKeybindPressed(CONFIG.DROP_V2_KEYBIND,input)then
                task.spawn(executeDropV2)
