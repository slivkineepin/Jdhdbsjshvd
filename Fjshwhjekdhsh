...
function runDropBrainrot()
    ...
    task.spawn(function()
        local colConn = S.RunService.Stepped:Connect(function()
...
local function findAnyTool()
    ...
    local bp = S.LocalPlayer:FindFirstChildOfClass("Backpack")
...
local function tryHitBat()
    ...
    pcall(function()
        local c = character
        if not c then _batCooldown = false; return end
...
-- ======================================
-- LOCK TARGET SYSTEM (Plasma logic)
-- ======================================
...
local function getClosestPlayer()
    local c = character; if not c then return nil, math.huge end
    ...
    lockTargetConnection = S.RunService.Heartbeat:Connect(function()
        ...
        if target and target.Character then
            ...
            if tr then
                ...
                local dir = (fp - myH.Position).Unit
                myH.AssemblyLinearVelocity = Vector3.new(dir.X * VYSE_AIMBOT_SPEED, dir.Y * VYSE_AIMBOT_SPEED, dir.Z * VYSE_AIMBOT_SPEED)
                if dist <= VYSE_HIT_DIST and CONFIG.AUTO_BAT_ENABLED then
                    tryHitBat()
                end
            end
        else
...
function stopBatAimbot()
    ...
    if myH then myH.AssemblyLinearVelocity = Vector3.zero end
    _batCooldown = false
end
...
function apStartLoop()
    ...
    AP.loopConn = S.RunService.Heartbeat:Connect(function()
        ...
        -- Float management: enable when moving to points 3, 4, 5
        if AP.currentStep >= 3 and AP._prevStep < 3 then
            startFloat()
        end
        -- Disable float when returning to point 4 from a higher step
        if AP.currentStep == 4 and AP._prevStep > 4 then
            stopFloat()
        end
        AP._prevStep = AP.currentStep
...
function startSpeed()
    ...
    speedBoostConn = S.RunService.RenderStepped:Connect(function()
        ...
        -- Don't apply speed during bat aimbot or auto play (File B style)
        if CONFIG.LOCK_TARGET_ENABLED and lockTargetConnection then return end
        if AP.enabled and AP.loopConn then return end
        ...
        elseif CONFIG.ANTIRAGDOLL_ENABLED and _lastMoveDir.Magnitude > 0 then
            ...
            for key in pairs(MOVE_KEYS) do
                if S.UserInputService:IsKeyDown(key) then anyHeld = true; break end
            end
            if anyHeld then
                root.Velocity = Vector3.new(
                    _lastMoveDir.X * vel,
...
local function setupFloatingPanels()
    ...
    _ltOnTap(function()
        ...
        if newState then
            ...
            -- Mutual exclusion: disable anti lock when enabling lock target
        ...
        saveConfig()
        if lockTargetEnabled then startBatAimbot() else stopBatAimbot() end
    end)
    ...
    _alockOnTap(function()
        ...
        -- Mutual exclusion: disable lock target when enabling anti lock
        if antiLockEnabled and CONFIG.LOCK_TARGET_ENABLED then
            CONFIG.LOCK_TARGET_ENABLED = false
            lockTargetEnabled = false
            stopBatAimbot()
            pcall(function()
...
local function createGui()
    ...
    S.UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        elseif isKeybindPressed(CONFIG.LOCK_TARGET_KEYBIND, input) then
            ...
            if newKbState then
                isSwitching = true                if AP.enabled then
                    apStopLoop()
                    apResetBtns()
                end
                isSwitching = false
