...
_duelToggleBtns={}
_duelTpAutoOn=false
_duelTpManualOn=false
_duelTpCooldown=false
_duelTpInProgress=false
_duelManualTpGui=nil
_duelManualKeybind=Enum.KeyCode.F
_duelWaitingForKeybind=false
TP_DUEL_LEFT=Vector3.new(-476.06,-6.64,94.73)
TP_DUEL_RIGHT=Vector3.new(-476.06,-6.64,26.00)
_discordBB=nil
...
function getDuelSide()
    local char=LP.Character
    local hrp=char and char:FindFirstChild("HumanoidRootPart")
    if not hrp then return "LEFT" end
    local midZ=(TP_DUEL_LEFT.Z+TP_DUEL_RIGHT.Z)/2
    return hrp.Position.Z>=midZ and "LEFT" or "RIGHT"
end
function isDuelRagdolled()
...
function fireDuelTp(side)
    _duelTpCooldown=true;_duelTpInProgress=true
    task.spawn(function()
        task.wait(0.1)
        local c=LP.Character
        local r=c and c:FindFirstChild("HumanoidRootPart")
        if not r then _duelTpCooldown=false;_duelTpInProgress=false;return end
        r.Velocity=Vector3.new(0,0,0)
        task.wait(0.05)
        r.CFrame=CFrame.new(side=="LEFT" and TP_DUEL_LEFT or TP_DUEL_RIGHT)
        task.wait(0.18)
        r.Velocity=Vector3.new(0,0,0)
        _duelTpInProgress=false
        task.wait(1.5)
...
function doManualDuelTp()
    if _duelTpCooldown or _duelTpInProgress then return end
    fireDuelTp(getDuelSide())
end
...
function startLockTarget()
    ...
    -- Drop Lock: если мы в Stealing и Drop Lock включён — сначала Drop, потом Lock
    ...
    -- Melee Aimbot: запомнить состояние и выключить при включении Lock V1
    if _maEnabled then
...
function stopLockTarget()
    ...
    _ltLockedTarget=nil
    _ltDisableAntiDie()
    -- Вернуть Melee Aimbot если он был включён до Lock V1
...
local function _lt2FindAnyTool()
    local c = LP.Character
    if c then
...
function startLockTarget2()
    ...
    -- Drop Lock check for LT2 - check only our character
    if CONFIG.DROP_LOCK_ENABLED and not _dropLockBusy then
        if LP:GetAttribute("Stealing") then
            _dropLockBusy = true
            task.spawn(function()
        ...
        if target and target.Character then
            ...
            if targetRoot then
                ...
                if cam then
                    ...
                    local targetPos = targetRoot.Position + (moveDir * 10)
...
-- ── Melee Aimbot ─────────────────────────────────────────────────────────────
local function _maGetClosest(myHRP, radius)
    local cp, cd = nil, radius
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and p.Character then
            local tr  = p.Character:FindFirstChild("HumanoidRootPart")
...
function startMeleeAimbot()
    ...
    _maConn = RunService.RenderStepped:Connect(function()
        ...
        local target = _maGetClosest(hrp, range)
        if target then
            ...
            local predicted   = tPos + vel * predictTime
            local lookPos     = Vector3.new(predicted.X, myPos.Y, predicted.Z)
            local toTarget    = lookPos - myPos
            if toTarget.Magnitude > 0.1 then
                ...
                hrp.RotVelocity = hrp.CFrame:VectorToWorldSpace(
                    Vector3.new(0, ry * 42, 0)
                )
            end
        else
...
local _STEAL_HOLD_MIN=1.35

local function _tryInitAdvancedSteal()
    local success=pcall(function()
        ...
        local _allAnimalsCache={}
        local _promptMemoryCache={}
        local _internalStealCache={}
        ...
        local function findProximityPromptForAnimal(animalData)
            ...
            local cached=_promptMemoryCache[animalData.uid]
            ...
            for _,p in ipairs(attach:GetChildren())do
                if p:IsA("ProximityPrompt")then
                    _promptMemoryCache[animalData.uid]=p
                    return p
                end
            end
            return nil
        ...
        local function distToAnimal(animalData)
            ...
            if not chr then return math.huge end
            local hrp=chr:FindFirstChild("HumanoidRootPart")or chr:FindFirstChild("UpperTorso")
        ...
        local function pickClosest()
            ...
            for _,animalData in ipairs(_allAnimalsCache)do
                if isMyBaseAnimal(animalData)then continue end
                local pos=getAnimalPosition(animalData)
                if not pos then continue end
                local dist=(hrp.Position-pos).Magnitude
                if dist>(CONFIG.AUTO_STEAL_SCAN_RANGE or 60) then continue end
        ...
        local function buildStealCallbacks(prompt)
            ...
            if(#data.holdCallbacks>0)or(#data.triggerCallbacks>0)then
                _internalStealCache[prompt]=data
            end
        end

        local STEAL_CFG={
            HOLD_MIN=_STEAL_HOLD_MIN,
            HOLD_MAX=math.max(CONFIG.AUTO_STEAL_DURATION or 2.0,_STEAL_HOLD_MIN),
            ENTRY_DELAY=0.3,
            COOLDOWN=0.05,
            TRIGGER_RANGE=CONFIG.AUTO_STEAL_ACTIVATION_DIST or 9,
        }

        local function executeStealAsync(prompt,animalData)
            local data=_internalStealCache[prompt]
            if not data or not data.ready then return false end
            data.ready=false
            STEAL_CFG.HOLD_MAX=math.max(CONFIG.AUTO_STEAL_DURATION or 2.0,_STEAL_HOLD_MIN)
            STEAL_CFG.TRIGGER_RANGE=CONFIG.AUTO_STEAL_ACTIVATION_DIST or 9
            local label=animalData.name or"Animal"
            _stealAutoState.active=true
            _stealAutoState.startTime=tick()
            _stealAutoState.phase="holding"
            _stealAutoState.label=label
            task.spawn(function()
                for _,fn in ipairs(data.holdCallbacks)do task.spawn(fn)end
                task.wait(STEAL_CFG.HOLD_MIN)
                _stealAutoState.phase="waitingRange"
                local alreadyInRange=distToAnimal(animalData)<=STEAL_CFG.TRIGGER_RANGE
                local fired=false
                while true do
                    local elapsed=tick()-_stealAutoState.startTime
                    if elapsed>STEAL_CFG.HOLD_MAX then break end
                    if not prompt.Parent then break end
                    if distToAnimal(animalData)<=STEAL_CFG.TRIGGER_RANGE then
                ...
                _stealAutoState.active=false
                _stealAutoState.phase="idle"
        ...
        local function attemptSteal(prompt,animalData)
            ...
            if not _internalStealCache[prompt] then return false end
        ...
        local function scanAllPlots()
            ...
            _allAnimalsCache=newCache
            return #_allAnimalsCache
        end

        local function startSteal()
            if _stealConnection then return end
            _stealConnection=RunService.Heartbeat:Connect(function()
                if not CONFIG.AUTO_STEAL_ENABLED then return end
                if _stealAutoState.active then return end
                local target=pickClosest()
                if not target then return end
                local prompt=_promptMemoryCache[target.uid]
        ...
        local function stopSteal()
            if _stealConnection then _stealConnection:Disconnect();_stealConnection=nil end
        ...
        startAutoSteal=startSteal
        stopAutoSteal=stopSteal
...
RunService.RenderStepped:Connect(function()
    ...
    if _stealAutoState.active then
        local elapsed=tick()-_stealAutoState.startTime
        if _stealAutoState.phase=="holding" then
            local prog=math.clamp(elapsed/_STEAL_HOLD_MIN,0,1)
            _stealPbFill.Size=UDim2.new(prog,0,1,0)
            _stealPbStatus.Text="Stealing "..math.floor(prog*100).."%"
            _stealPbFill.BackgroundColor3=COLORS.Accent
        elseif _stealAutoState.phase=="waitingRange" then
            _stealPbFill.Size=UDim2.new(1,0,1,0)
            _stealPbStatus.Text="Waiting"
            _stealPbFill.BackgroundColor3=Color3.fromRGB(255,180,60)
        end
    else
        _stealPbFill.Size=UDim2.new(0,0,1,0)
        _stealPbStatus.Text="Ready"
        _stealPbFill.BackgroundColor3=COLORS.Accent
    end
end)
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        do
            ...
            -- Tracks whether speed booster was ON before carry was activated
            local _carrySpeedWasOn=false
            _carryOnTap(function()
                if _carryEnabled then
                    -- Turning carry OFF
                    stopCarry()
                    -- Restore speed booster if it was on before carry was turned on
                    if _carrySpeedWasOn then
                        _carrySpeedWasOn=false
                        CONFIG.SPEED_ENABLED=true
                        speedGuiEnabled=true
                        CONFIG.SPEED_GUI_VISIBLE=true
                        if speedGuiInstance then speedGuiInstance.Enabled=true end
                        stopSpeed()
                        startSpeed()
                    end
                else
                    -- Turning carry ON: remember if speed booster is currently enabled
                    _carrySpeedWasOn=CONFIG.SPEED_ENABLED==true
                    if _carrySpeedWasOn then
                        -- Disable speed booster so carry uses steal speed instead
                        CONFIG.SPEED_ENABLED=false
                        speedGuiEnabled=false
                        CONFIG.SPEED_GUI_VISIBLE=false
                        if speedGuiInstance then speedGuiInstance.Enabled=false end
                        stopSpeed()
                    end
                    startCarry()
                end
        ...
        do
            local _mtpGui,_mtpBtn,_mtpOnTap=createFloatingPanel("ManualTp","manualTpPanel","MANUAL_TP_PANEL_VISIBLE","MANUAL TP")
            _duelManualTpGui=_mtpGui
            _mtpOnTap(function()doManualDuelTp()end)
        end
    ...
    local function createDuelTab(parent)
        ...
        autoStealToggle.LayoutOrder=5
        local igDistInput=createNumberInput(toolsContent,"Full Steal Range",CONFIG.AUTO_STEAL_ACTIVATION_DIST,0.1,9999,function(value)
            CONFIG.AUTO_STEAL_ACTIVATION_DIST=value
        end,true)
        igDistInput.LayoutOrder=6
        local igScanInput=createNumberInput(toolsContent,"Stealing Range",CONFIG.AUTO_STEAL_SCAN_RANGE,1,9999,function(value)
            CONFIG.AUTO_STEAL_SCAN_RANGE=value
        end,true)
        igScanInput.LayoutOrder=7
        local stealDurInput=createNumberInput(toolsContent,"Hold Duration (0=1.3s)",CONFIG.AUTO_STEAL_DURATION-1.3,0,1.3,function(value)
            local real=math.clamp(value,0,1.3)+1.3
            CONFIG.AUTO_STEAL_DURATION=real
        end,true)
        stealDurInput.LayoutOrder=8
        table.insert(_duelToggleBtns,{btn=_igBtn,default=false})
        local dropBrainrotPanelToggle,_dropBtn=createPanelShowBtn(toolsContent,"Drop Brainrot","DROP_BRAINROT_PANEL_VISIBLE",function()return dropBrainrotPanelGui end)
        dropBrainrotPanelToggle.LayoutOrder=10
        ...
        table.insert(_duelToggleBtns,{btn=_arBtn,default=false})
        local tpContent,tpWrapper=createCategory(scroll, "TP",true)
        tpWrapper.LayoutOrder=4
        local autoTpToggle,_autoTpBtn=createToggle(tpContent,"Auto TP",false,function(state)
            _duelTpAutoOn=state
        end)
        autoTpToggle.LayoutOrder=1
        table.insert(_duelToggleBtns,{btn=_autoTpBtn,default=false})
        local manualTpToggle,_manualTpBtn=createToggle(tpContent,"Manual TP",CONFIG.MANUAL_TP_PANEL_VISIBLE,function(state)
            _duelTpManualOn=state
            CONFIG.MANUAL_TP_PANEL_VISIBLE=state
            if _duelManualTpGui then _duelManualTpGui.Enabled=state end
        end)
        manualTpToggle.LayoutOrder=2
        table.insert(_duelToggleBtns,{btn=_manualTpBtn,default=false})
        ...
        resetBtn.MouseButton1Click:Connect(function()
            ...
            local defaults={
                ...
                TP_DOWN_PANEL_VISIBLE=false,AUTO_TP_DOWN_PANEL_VISIBLE=false,MANUAL_TP_PANEL_VISIBLE=false,AUTO_WALK_PANEL_VISIBLE=false,
                AUTO_PLAY_GUI_VISIBLE=false,
                INSTANT_RESET_PANEL_VISIBLE=false,
    ...
    local function reapplyAllSettings()
        ...
        if CONFIG.MANUAL_TP_PANEL_VISIBLE and _duelManualTpGui then
            _duelManualTpGui.Enabled=true
            _duelTpManualOn=true
        end
    ...
    RunService.Heartbeat:Connect(function()
        if _duelTpCooldown or _duelTpInProgress then return end
        if not _duelTpAutoOn then return end
        if not isDuelRagdolled() then return end
        fireDuelTp(getDuelSide())
    end)
    UIS.InputBegan:Connect(function(inp,gpe)
        if gpe or _duelWaitingForKeybind then return end
        if inp.KeyCode==_duelManualKeybind and _duelTpManualOn then
            doManualDuelTp()
        end
    end)
