...
CONFIG={
    ...
    BYPASS_ANTI_BAT_LOCK_KEYBIND=nil,
    BYPASS_ANTI_BAT_LOCK_ENABLED=false,
    BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE=false,
...
_KEYBIND_SET={
    ...
    AUTO_UP_KEYBIND=true,ANTI_BAT_KEYBIND=true,BYPASS_ANTI_BAT_LOCK_KEYBIND=true,BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE=true,
}
...
_bypassAblPanelGui,_bypassAblPanelBtn=nil,nil
...
-- BYPASS ANTI BAT LOCK
local _bypassAblEnabled = false
local _bypassAblConn = nil

local function _bypassAblTick()
    if not _bypassAblEnabled then return end
    local char = LP.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
    local hum = char:FindFirstChildOfClass("Humanoid")
    ...
    if math.abs(v.Y) > 80 and not CONFIG.LOCK_TARGET_ENABLED then
        hrp.AssemblyLinearVelocity = Vector3.new(v.X, math.clamp(v.Y, -80, 80), v.Z)
    end
    for _, d in ipairs(char:GetDescendants()) do
        local n = d.Name:lower()
        if n:find("batlock") or n:find("batfreeze") or n:find("stunlock") or n:find("lockbat") then
            pcall(function() d:Destroy() end)
        end
    end
end

local function startBypassAbl()
    if _bypassAblConn then return end
    _bypassAblEnabled = true
    _bypassAblConn = RunService.Heartbeat:Connect(_bypassAblTick)
end

local function stopBypassAbl()
    _bypassAblEnabled = false
    if _bypassAblConn then _bypassAblConn:Disconnect(); _bypassAblConn = nil end
end
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        local _bablOnTap
        _bypassAblPanelGui,_bypassAblPanelBtn,_bablOnTap=createFloatingPanel("BypassABL","bypassAblPanel","BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE","ABL: "..(CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED and"ON"or"OFF"))
        trackLabel(_bypassAblPanelBtn)
        local function _syncBypassAblPanel(state)
            if _bypassAblPanelBtn then _bypassAblPanelBtn.Text="ABL: "..(state and"ON"or"OFF") end
        end
        if CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED then _syncBypassAblPanel(true) end
        _bablOnTap(function()
            local s=not CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED
            CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED=s
            _syncBypassAblPanel(s)
            if s then startBypassAbl() else stopBypassAbl() end
        end)
        local _flOnTap
    ...
    local function createSettingsTab(parent)
        ...
        local floatingPanelsToggle=createToggle(uiContent,"Floating Panels",CONFIG.FLOATING_PANELS_ENABLED,function(state)
            ...
            local panels={
                lockTargetPanelGui,spinbotPanelGui,floatPanelGui,
                awGuiInstance,_atdGuiInstance,auGuiInstance,apGuiInstance,
                speedGuiInstance,dropBrainrotPanelGui,tpDownPanelGui,
                _instantResetPanelGui,antiBatPanelGui,tauntPanelGui,_bypassAblPanelGui,
            }
            ...
            if state then
                ...
                if CONFIG.BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE and _bypassAblPanelGui then _bypassAblPanelGui.Enabled=true end
    ...
    local function createBindsTab(parent)
        ...
        kb("Bypass Anti Bat Lock","BYPASS_ANTI_BAT_LOCK_KEYBIND",17)
        return tab
    end
    ...
    local function createDuelTab(parent)
        ...
        autoAntiBatToggle.LayoutOrder=8
        ...
        local bypassAblToggle,_bypassAblBtn2=createPanelShowBtn(combatContent,"Bypass Anti Bat Lock","BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE",function()return _bypassAblPanelGui end)
        bypassAblToggle.LayoutOrder=11
        table.insert(_duelToggleBtns,{btn=_bypassAblBtn2,default=false})
    ...
    local function createGui()
        ...
        UIS.InputBegan:Connect(function(input,gpe)
            ...
            elseif isKeybindPressed(CONFIG.BYPASS_ANTI_BAT_LOCK_KEYBIND,input)then
                local s=not CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED
                CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED=s
                if _bypassAblPanelBtn then _bypassAblPanelBtn.Text="ABL: "..(s and"ON"or"OFF") end
                if s then startBypassAbl() else stopBypassAbl() end
                showNotification("Bypass Anti Bat Lock",s)
    ...
    local function reapplyAllSettings()
        ...
        if CONFIG.BYPASS_ANTI_BAT_LOCK_PANEL_VISIBLE and _bypassAblPanelGui then
            _bypassAblPanelGui.Enabled=true
        end
        end
        ...
        if CONFIG.BYPASS_ANTI_BAT_LOCK_ENABLED then
            startBypassAbl()
            task.wait(0.05)
        end
        pcall(function()
