...
function runDropBrainrot()
    ...
    dc=RunService.Heartbeat:Connect(function()
        ...
        if not r then dc:Disconnect();dropBrainrotActive=false;if dropBrainrotPanelBtn then dropBrainrotPanelBtn.Text="DROP BRAINROT"end;return end
...
function startLockTarget()
    ...
    _ltCharAddedConn=LP.CharacterAdded:Connect(function(ch)
        ...
        lockTargetConnection=RunService.RenderStepped:Connect(function()
            ...
            local targetPos,targetCF=getRealTargetPosition(_ltLockedTarget)
            local targetChar=_ltLockedTarget and _ltLockedTarget.Character
            local targetHRP=targetChar and targetChar:FindFirstChild("HumanoidRootPart")
            if targetPos and targetHRP then
...
function startInfJump()
    ...
    infJumpConnection=UIS.JumpRequest:Connect(function()
        ...
        local now=tick()
        if now-_ijLastJump<0.15 then return end
        _ijLastJump=now
        local char=character
        if not char then return end
        local hrp=char:FindFirstChild("HumanoidRootPart")
        ...
        hrp.Velocity=Vector3.new(vel.X,jumpForce,vel.Z)
    end)
...
function stopAntiLag()
    -- FPS boost is one-time apply, cannot fully revert
...
local function _doJumpBoostHop()
    ...
    pcall(function()
        ...
        if hum.FloorMaterial==Enum.Material.Air then
            h.Velocity=Vector3.new(h.Velocity.X,CONFIG.JUMP_BOOST_HOP_POWER,h.Velocity.Z)
        end
    end)
end
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        _abOnTap(function()
            ...
            CONFIG.ANTI_BAT_ENABLED=newState
            _syncAntiBatPanel(newState)
            if _antiBatBtn then setToggleVisual(_antiBatBtn,newState)end
            saveConfig()
        end)
        local _flOnTap
            ...
            for _,gui in ipairs(panels)do
                if gui then
                    if not state then gui.Enabled=false
                        ...
                        -- restore each panel based on its own config
                    end
                end
            end
            if state then
    ...
    local function createDuelTab(parent)
        ...
        local antiBatToggle,_antiBatBtnLocal=createToggle(combatContent,"Anti Bat",CONFIG.ANTI_BAT_ENABLED,function(state)
            CONFIG.ANTI_BAT_ENABLED=state;saveConfig()
            if antiBatPanelBtn then antiBatPanelBtn.Text="BAT: "..(state and"ON"or"OFF")end
            if antiBatPanelGui then antiBatPanelGui.Enabled=true end
        end)
