...
do
    ...
    local _Kicks = { new = plr.Kick, old = plr.kick }
    ...
    local function _runKickChecks()
        local cases = {
            ...
            function()
                ...
                for _, mm in {"__eq","__lt","__le","__add","__sub","__mul","__div",
                              "__mod","__pow","__unm","__len","__call","__index",
                              "__concat","__tostring","__newindex"} do
                    mt[mm] = error
                end
                for _, k in _Kicks do k(obj) end
            end,
            function() plr:kIcK() end,
        }
...
local function makeDraggable(frame, handle, saveKey, lockedFn, onSaved)
...
local function loadAndClampPosition(frame, key, defaultPos)
...
local _KEYBIND_SET = {
    ...
    AUTO_REACT_STEAL_KEYBIND=true,
}

local _TABLE_KEYS = { _guiPositions=true, _autoPlayOffsets=true, _apBasePositions=true }

local _savePending = false

local function _doWrite()
    pcall(function()
...
local medusaCounterConns = {}
local MEDUSA_COUNTER_COOLDOWN = 25
local medusaCounterLastUsed = 0
local medusaCounterDebounce = false
local antiDieEnabled = false

local speedGuiEnabled = false
local speedGuiInstance = nil
local laggerGuiInstance = nil
local _laggerEnabled = false
local _laggerLoopThread = nil
...
-- Side detection happens on-demand when Auto Play / Auto Walk is activated
...
local _ltLockedTarget        = nil    -- locked target for new lock target chase
local _ltLastTargetY         = nil    -- previous Y of locked target HRP (tp-down detector)
local _ltTpMirrorInProgress  = false  -- re-entry guard for mirror tp-down
local _fastPanelBurgerFrame = nil
local _espHitboxes = {}
-- Registry for resettable draggable frames: {frame, defaultPosFn}
local _draggableRegistry = {}
local function _regDraggable(frame, defaultPosFn)
...
local _duelToggleBtns = {}   -- { {btn, defaultOn} }
-- speed meter (k7 style)
local _speedMeterBB         = nil   -- BillboardGui on head
local _speedMeterLabel      = nil   -- TextLabel inside BB
local _speedMeterUpdateConn = nil   -- RenderStepped connection
local _speedMeterCharConn   = nil   -- CharacterAdded reconnect
local _discordBB            = nil   -- BillboardGui: discord label above head
local _discordCharConn      = nil   -- CharacterAdded reconnect for discord BB
...
local _sideDetected = false
local _currentSide  = nil

-- Known world-space positions of each PlotSign (plots 3 and 7)
local _PLOT_POSITIONS = {
...
local _PLOT_TO_SIDE = { [3] = "L", [7] = "R" }

local antiDieConnections = {}

local _arV2Conn    = nil
local _arV2Enabled = false
...
local _savedOrigAnims = nil
local _animHeartbeatConn = nil
...
local _lockAlignOri = nil
local _lockAttach   = nil
...
local desyncLoopConn = nil
local desyncPanelGui = nil
local desyncPanelBtn = nil
-- Server position ESP (desync ghost)
local _desyncEspCube        = nil
local _desyncEspHB          = nil

local DesyncSettings = {
    ["Rendering.S2PhysicsSenderRate"] = 1,
    ["Rendering.SimRadius"]           = 0,
}

local floatModeConn     = nil
local floatEnabled      = false
local _floatCurHip      = 0
local _floatTpInProgress = false
local _floatGeneration  = 0
...
local autoReactStealEnabled = false
local _autoReactStealConn = nil
local _autoReactStealCooldown = false

local REACT_STEAL_KEYWORD = "someone is stealing"

local _fastPanelGui      = nil
local _fastPanelGuiR     = nil  -- right panel ScreenGui
local _fastPanelVisible  = false
...
local function _getBindDisplayText(bind)
...
local function isKeybindPressed(bind, input)
...
local function _getPlotOwnerText(plotSign)
...
local function _waitMyPlot()
    repeat task.wait(0.5) until _getMyPlot() ~= nil
end


local function _detectSide()
...
local function deactivateAntiDie()
...
local function toggleAntiRagdollV2(state)
    _arV2Enabled = state
...
local function createNumberInput(parent, labelText, defaultValue, minVal, maxVal, callback, allowFloat)
    minVal = minVal or 0
...
local function executeDrop() task.spawn(runDropBrainrot) end
...
local function _saveOriginalAnims(char)
...
local function _applyHarderHitAnimPack(char)
    local animate = char:FindFirstChild("Animate"); if not animate then return end
...
local function startBodyAimbot()
    ...
    aimbotConnection = S.RunService.RenderStepped:Connect(function()
        ...
        -- Flat логика по умолчанию (только горизонталь)
        -- При включённом Lock и дистанции <= AIMBOT_FLAT_REMOVE_DIST — полное 3D преследование
...
local function _ltIsValid(char)
    if not char then return false end
...
local function _ltGetTarget(myHRP)
...
local function _stopLockLookAt()
...
-- ======================================
-- OPTIMIZER + ANTI-LAG (merged, full BKHUB methods)
-- ======================================


local function _applyFFlags()
...
-- Anti-lag: strip accessories/clothing/effects from a character
local function _antiLagCleanChar(char)
    if not char then return end
    pcall(function()
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Accessory") or child:IsA("Hat") or
               child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") or
               child:IsA("CharacterMesh") or child:IsA("BodyColors") then
                pcall(function() child:Destroy() end)
            end
        end
        for _, desc in ipairs(char:GetDescendants()) do
            pcall(function()
                if desc.ClassName == "LayeredClothing" or desc.ClassName == "WrapLayer" or
                   desc:IsA("ParticleEmitter") or desc:IsA("Trail") or desc:IsA("Beam") or
                   desc:IsA("PointLight") or desc:IsA("SpotLight") or desc:IsA("SurfaceLight") or
                   desc:IsA("Fire") or desc:IsA("Smoke") or desc:IsA("Sparkles") or
                   desc:IsA("Highlight") then
                    desc:Destroy()
                elseif (desc:IsA("Decal") or desc:IsA("Texture")) and
                       not (desc.Name == "face" and desc.Parent and desc.Parent.Name == "Head") then
                    desc:Destroy()
    ...
    _cleanedChars[char] = true
end

local function _antiLagCleanBackpack(plr)
    pcall(function()
...
local function _startAntiLag()
    if _antiLagRunning then return end
    _antiLagRunning = true
    _cleanedChars   = {}

    for _, plr in ipairs(S.Players:GetPlayers()) do
        if plr.Character then
            _antiLagCleanChar(plr.Character)
            _antiLagCleanBackpack(plr)
        end
        table.insert(_antiLagConns, plr.CharacterAdded:Connect(function(char)
            if not _antiLagRunning then return end
            task.wait(0.5)
            _antiLagCleanChar(char)
            _antiLagCleanBackpack(plr)
            table.insert(_antiLagConns, char.ChildAdded:Connect(function(child)
                if not _antiLagRunning then return end
                task.wait(0.1)
                if child:IsA("Accessory") or child:IsA("Hat") or
                   child:IsA("Shirt") or child:IsA("Pants") or child:IsA("ShirtGraphic") then
                    pcall(function() child:Destroy() end)
                end
            end))
        end))
        if plr.Backpack then
            table.insert(_antiLagConns, plr.Backpack.ChildAdded:Connect(function()
                if _antiLagRunning then task.wait(0.1); _antiLagCleanBackpack(plr) end
            end))
        end
    end

    table.insert(_antiLagConns, S.Players.PlayerAdded:Connect(function(plr)
        table.insert(_antiLagConns, plr.CharacterAdded:Connect(function(char)
            if not _antiLagRunning then return end
            task.wait(0.5)
            _antiLagCleanChar(char)
    ...
    -- Periodic cleanup every 3s for any missed characters
    table.insert(_antiLagConns, task.spawn(function()
        while _antiLagRunning do
            task.wait(3)
            for _, plr in ipairs(S.Players:GetPlayers()) do
                if plr.Character and not _cleanedChars[plr.Character] then
                    _antiLagCleanChar(plr.Character)
                    _antiLagCleanBackpack(plr)
                end
            end
...
local function _stopAntiLag()
    _antiLagRunning = false
    for _, c in ipairs(_antiLagConns) do
        pcall(function()
...
local function startOptimizer()
    ...
    pcall(function()
        ...
        for _, cls in ipairs({"Atmosphere","BloomEffect","BlurEffect","ColorCorrectionEffect","SunRaysEffect","DepthOfFieldEffect"}) do
            local obj = S.Lighting:FindFirstChildOfClass(cls)
    ...
    -- Nuke existing visual effects after 1 sec
    table.insert(_optThreads, task.spawn(function()
        task.wait(1)
        _nukeVisuals()
    end))

    -- Watch for new visual effects and destroy them
...
local function enableNoCameraCollision()
    ...
    noCamCollisionConn = S.RunService.RenderStepped:Connect(function()
        ...
        for _ = 1, 12 do
            ...
            if p and p:IsA("BasePart") and not p:IsDescendantOf(ch) then
                hit[p] = true
                if noCamParts[p] == nil then
                    -- save independently — never reads from originalTransparency (XRay)
                    noCamParts[p] = p.LocalTransparencyModifier
                end
        ...
        local toRestore = {}
        for p in pairs(noCamParts) do
            if not hit[p] then table.insert(toRestore, p) end
        end
        for _, p in ipairs(toRestore) do
            pcall(function()
                if p and p.Parent then p.LocalTransparencyModifier = noCamParts[p] end
            end)
            noCamParts[p] = nil
        end
    end)
...
local function executeDesyncPayload()
    pcall(function()
        local ReplicatedStorage = S.ReplicatedStorage
        local Packages = ReplicatedStorage:WaitForChild("Packages", 5)
        local Net = Packages and Packages:FindFirstChild("Net")
        if Net then
            local UseItemRemote    = Net:FindFirstChild("RE/UseItem")
            local TeleportRemote   = Net:FindFirstChild("RE/QuantumCloner/OnTeleport")
            for flagName, flagValue in pairs(DesyncSettings) do
                pcall(function()
                    setfflag(tostring(flagName), tostring(flagValue))
                end)
            end
            if UseItemRemote  then pcall(function() UseItemRemote:FireServer()  end) end
            if TeleportRemote then pcall(function() TeleportRemote:FireServer() end) end
        end
    end)
end

local function startDesync()
    executeDesyncPayload()
...
local function startServerPos(char)
...
-- Синхронизировать текст кнопки Float с текущим состоянием
local function _syncFloatBtn(active)
    if not floatPanelBtn then return end
    floatPanelBtn.Text       = active and "FLOAT: ON" or "FLOAT: OFF"
    floatPanelBtn.TextColor3 = COLORS.Accent  -- всегда акцентный
    pcall(function() trackLabel(floatPanelBtn) end)
end
...
local function startFloat()
    ...
    local myGen = _floatGeneration  -- снимок поколения при старте

    floatModeConn = S.RunService.Heartbeat:Connect(function()
        if not floatEnabled then return end
        if _floatTpInProgress then return end
        pcall(function()
            local char = character
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            ...
            -- Auto TP Down: пауза через флаг + генерация для отмены
            if CONFIG.AUTO_TP_DOWN_ENABLED
               and _floatCurHip >= target - 0.05 then
                ...
                task.delay(0.05, function()
                    -- если float выключили пока ждали — отмена
                    if not floatEnabled or _floatGeneration ~= myGen then
                        _floatTpInProgress = false
                        return
                    end
                    tpDown()
                    task.delay(0.08, function()
                        -- ещё раз проверяем — вдруг выключили между delay
                        if _floatGeneration ~= myGen then
                            _floatTpInProgress = false
                            return
                        end
                        _floatTpInProgress = false
...
local function stopFloat()
    ...
    _floatTpInProgress  = false
    _floatGeneration    = _floatGeneration + 1  -- инвалидирует все pending task.delay
...
local function findMedusaForCounter()
...
local function createFloatingPanel(name, configPath, configVisible, defaultText)
    ...
    makeDraggable(frame, frame, configPath, function() return CONFIG.UI_LOCKED end)
...
local function _speedMeterSetupBB()
...
local function _discordBBSetup()
...
local function _startDiscordBB()
    _discordBBSetup()
...
local function createSpeedGui()
    ...
    local _speedValBox, _stealValBox = nil, nil
...
local function createFastPanel()
    ...
    -- Sync float badge with floatEnabled state
    task.spawn(function()
        local last = nil
        while mainR and mainR.Parent do
            task.wait(0.15)
            local cur = floatEnabled
    ...
    brgFrame.InputEnded:Connect(function(inp)
        ...
        _fastPanelVisible = not _fastPanelVisible
        mainL.Visible = _fastPanelVisible
        mainR.Visible = _fastPanelVisible
...
local function setupFloatingPanels()
    ...
    floatPanelGui, floatPanelBtn = createFloatingPanel("Float", "floatPanel", "FLOAT_PANEL_VISIBLE", "FLOAT: " .. (CONFIG.FLOAT_ACTIVE and "ON" or "OFF"))

    _syncFloatBtn(CONFIG.FLOAT_ACTIVE)

    floatPanelBtn.Activated:Connect(function()
        if not CONFIG.FLOAT_PANEL_VISIBLE then return end  -- панель должна быть включена
    ...
    desyncPanelGui, desyncPanelBtn = createFloatingPanel(
    ...
    if CONFIG.DESYNC_ENABLED then
        S.TweenService:Create(desyncPanelBtn, tweenInfoFast, {
            BackgroundColor3       = COLORS.Accent,
            BackgroundTransparency = 0.0,
        }):Play()
        desyncPanelBtn.TextColor3 = Color3.fromRGB(10, 10, 10)
        -- Re-enable server-pos ESP cube if desync was active before reload
        task.spawn(function()
            task.wait(0.3)
            local char = S.LocalPlayer.Character
    ...
    desyncPanelBtn.Activated:Connect(function()
        local newState = not CONFIG.DESYNC_ENABLED
        -- Toggle desync
        toggleDesync(newState)
        -- Toggle player ESP alongside desync
        CONFIG.ESP_ENABLED = newState
        espEnabled = newState
        if newState then
            startESP()
        else
        ...
        -- Visual update
        S.TweenService:Create(desyncPanelBtn, tweenInfoFast, {
            BackgroundColor3       = newState and COLORS.Accent or Color3.fromRGB(12, 14, 20),
            BackgroundTransparency = newState and 0.0 or 0.15,
        }):Play()
        desyncPanelBtn.TextColor3 = newState and Color3.fromRGB(10, 10, 10) or COLORS.Accent
...
local function createSettingsTab(parent)
    ...
    local fastPanelToggle = createToggle(uiContent, "Fast Panel", CONFIG.FAST_PANEL_ENABLED, function(state)
        ...
        if not state then
            _fastPanelVisible = false
            if _fastPanelGui  then _fastPanelGui.Enabled  = false end
            if _fastPanelGuiR then _fastPanelGuiR.Enabled = false end
        else
            if _fastPanelGui  then _fastPanelGui.Enabled  = true end
            if _fastPanelGuiR then _fastPanelGuiR.Enabled = true end
        end
    end)
...
local function createDuelTab(parent)
    ...
    local floatPanelToggle, _floatBtn = createToggle(mobContent, "Float", CONFIG.FLOAT_PANEL_VISIBLE, function(state)
        ...
        if state then
            ...
            if floatEnabled then stopFloat() end
        end
    end)
    ...
    local antiStealToggle, _asBtn = createToggle(toolsContent, "Anti Steal", CONFIG.ANTI_STEAL_PANEL_VISIBLE, function(state)
        ...
        if antiStealPanelBtn then antiStealPanelBtn.Text = "ANTI STEAL: " .. (state and "ON" or "OFF") end
    end)
    ...
    local autoLockToggle, _alBtn = createToggle(toolsContent, "Auto Lock", CONFIG.AUTO_LOCK_PANEL_VISIBLE, function(state)
        ...
        if autoLockPanelBtn then autoLockPanelBtn.Text = "AUTO LOCK: " .. (state and "ON" or "OFF") end
    end)
    ...
    resetBtn.MouseButton1Click:Connect(function()
        ...
        for _, gui in ipairs({
            ...
            spinbotPanelGui, tauntPanelGui, antiStealPanelGui, flingPanelGui,
            autoLockPanelGui, tpDownPanelGui, awGuiInstance, apGuiInstance,
            desyncPanelGui, laggerGuiInstance,
        }) do
...
local function createGui()
    ...
    S.UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        elseif isKeybindPressed(CONFIG.FLOAT_KEYBIND, input) then
            if not CONFIG.FLOAT_PANEL_VISIBLE then return end
            if floatEnabled then stopFloat() else CONFIG.FLOAT_ACTIVE = true; startFloat() end
            saveConfig()
...
local function stopAllSystems()
    ...
    stopUnwalk()
    unwalkEnabled = false
    -- esp (keep alive if desync owns it)
    if not CONFIG.DESYNC_ENABLED then
        stopESP()
        espEnabled = false
    end
    ...
    -- speed
    stopSpeed()
    -- float + tp down: disconnect conn + reset gen but KEEP floatEnabled flag for reapply
    _floatGeneration = _floatGeneration + 1
    _floatTpInProgress = false
    if floatModeConn then floatModeConn:Disconnect(); floatModeConn = nil end
    pcall(function()
        local c = S.LocalPlayer.Character
...
-- Применяем AP-оффсеты здесь, где AP уже инициализирован
if CONFIG._autoPlayOffsets then
    for _, side in ipairs({"L","R"}) do
...
-- Delay feature reapplication until GUI is fully rendered and character is ready
task.spawn(function()
    task.wait(0.5)
    reapplyAllSettings()
    -- Apply Harder Hit Anim globally
    pcall(function()
...
local function stripVisuals(obj)
    ...
    if obj:IsA("Animator") then
        ...
        if model then
            ...
            if game:GetService("Players"):GetPlayerFromCharacter(model) == _lp then return end
        end
        ...
        obj.AnimationPlayed:Connect(function(track)
            ...
            if m2 then
                ...
                if game:GetService("Players"):GetPlayerFromCharacter(m2) == _lp2 then return end
            end
            track:Stop(0)
        end)
    end
...
local function enableFPSBoost()
    ...
    Workspace.DescendantAdded:Connect(stripVisuals)
end
