...
local _LT_PREDICTION = 0.13
local _LT_DIST_THRESHOLD = 1.5

local function _ltFindNearest()
    local myC = character; if not myC then return nil, nil, math.huge end
    local myH = myC:FindFirstChild("HumanoidRootPart"); if not myH then return nil, nil, math.huge end
    local nearest, nearestTorso, nearestDist = nil, nil, math.huge
    for _, p in ipairs(S.Players:GetPlayers()) do
        if p ~= S.LocalPlayer and p.Character then
            ...
            if eh and hum and hum.Health > 0 then
                local d = (eh.Position - myH.Position).Magnitude
    ...
    return nearest, nearestTorso, nearestDist
end
...
function startLockTarget()
    ...
    lockTargetConnection = S.RunService.Heartbeat:Connect(function()
        ...
        local targetHRP, torso, dist = _ltFindNearest()
        if not targetHRP or not torso then return end
        local spd = CONFIG.LOCK_TARGET_SPEED or 55
        local predictedPos = torso.Position + (torso.AssemblyLinearVelocity * _LT_PREDICTION)
        local dir = predictedPos - myH.Position
        if dir.Magnitude > _LT_DIST_THRESHOLD then
            local moveDir = dir.Unit
            myH.AssemblyLinearVelocity = Vector3.new(
                moveDir.X * spd,
                myH.AssemblyLinearVelocity.Y,
                moveDir.Z * spd
            )
        else
            myH.AssemblyLinearVelocity = Vector3.new(
                targetHRP.AssemblyLinearVelocity.X,
                myH.AssemblyLinearVelocity.Y,
                targetHRP.AssemblyLinearVelocity.Z
            )
        end
    end)
