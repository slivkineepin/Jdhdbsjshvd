...
_speedMeterUpdateConn=nil
_speedMeterCharConn=nil
_discordBB=nil
...
function startLockTarget()
    ...
    lockTargetConnection=RunService.Heartbeat:Connect(function()
        if not CONFIG.LOCK_TARGET_ENABLED then return end
        local myC=character;if not myC then return end
        ...
        local targetHRP,targetChar=_ltGetBestTarget(myH)
        if targetHRP and targetChar then
            ...
            local dirToTarget=predictedPos-myH.Position
            local dist3D=dirToTarget.Magnitude
            local targetStandPos=dist3D>0 and(predictedPos-dirToTarget.Unit*_LT_STOP_DISTANCE)or predictedPos
            local moveDir=targetStandPos-myH.Position
            local distToStand=moveDir.Magnitude
            if distToStand>1 then
                myH.Velocity=moveDir.Unit*spd
            else
                myH.Velocity=targetVel
            end
        else
            _ltLockedTarget=nil
            myH.Velocity=Vector3.zero
        end
    end)
...
local function _stripVisuals(obj)
    ...
    if obj:IsA("Animator")then
        ...
        obj.AnimationPlayed:Connect(function(track)
            ...
            if m2 then
                local lp2=LP
                if Players:GetPlayerFromCharacter(m2)==lp2 then return end
            end
            track:Stop(0)
        end)
    end
...
function stopOptimizer()
    ...
    pcall(function()
        ...
        Lighting.GlobalShadows=_optOriginal.globalShadows~=false
        Lighting.Brightness=_optOriginal.brightness or 1
        Lighting.FogEnd=_optOriginal.fogEnd or 100000
        Lighting.FogStart=_optOriginal.fogStart or 0
        Lighting.Technology=_optOriginal.technology or Enum.Technology.ShadowMap
        Lighting.EnvironmentDiffuseScale=_optOriginal.envDiffuse or 1
        Lighting.EnvironmentSpecularScale=_optOriginal.envSpecular or 1
...
function enableNoCameraCollision()
    ...
    _NCC.conn=RunService.RenderStepped:Connect(function()
...
RunService.RenderStepped:Connect(function()
    if not AutoPlayState.floating then return end
    ...
    if result then
        local diff=(result.Position.Y+9)-hrp.Position.Y
...
local function moveTo(hrp,hum,target,speed,checkEnabled)
    repeat
        ...
        AutoPlayState.floating=(speed==CONFIG.SPEED_VALUE)
        ...
        local look=CFrame.lookAt(hrp.Position,Vector3.new(target.X,hrp.Position.Y,target.Z))
...
function createAutoPlayGui()
    ...
    makeAS(apBadge,1,0.35);trackLabel(apBadge)
    task.spawn(function()
...
local function _main()
    ...
    local function createSettingsTab(parent)
        ...
        fovLockBtn.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(fovLockBtn,tweenInfoFast,{
    ...
    function toggleSpeedMeter(state)
        ...
        _speedMeterSetupBB()
        _speedMeterCharConn=LP.CharacterAdded:Connect(function()
            task.wait(0.5)
            if CONFIG.SPEED_METER_ENABLED then _speedMeterSetupBB()end
        end)
        _speedMeterUpdateConn=RunService.RenderStepped:Connect(function()
            ...
            local speed=Vector3.new(
                hrp.Velocity.X,0,hrp.Velocity.Z
            ).Magnitude
    ...
    do
        ...
        _setAutoLoad=function(v)
            ...
            if v then
                _kickConn=Players.PlayerRemoving:Connect(function(player)
