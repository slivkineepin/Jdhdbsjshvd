...
local function _lt2TryHit()
    if _lt2HittingCooldown then return end;_lt2HittingCooldown=true
    pcall(function()
        local bat=_lt2GetBat();if bat then
            bat:Activate();local ev=bat:FindFirstChildWhichIsA("RemoteEvent")
            if ev then ev:FireServer()end
        end
    end)
    task.delay(0.08,function()_lt2HittingCooldown=false end)
end
...
function startLockTarget2()
    ...
    _lt2HRPConn=RunService.Heartbeat:Connect(function()
        ...
        local target,_=_lt2GetClosest(hrp)
        if target and target.Character then
            local tr=target.Character:FindFirstChild("HumanoidRootPart")
            if tr then
                local dir=tr.Position-hrp.Position
                if dir.Magnitude>0.1 then
                    hrp.Velocity=dir.Unit*(CONFIG.LOCK_TARGET2_CHASE_SPEED or 58)
                end
                local cam=workspace.CurrentCamera
...
function startAntiLag()
    ...
    workspace.DescendantAdded:Connect(function(v)
        if CONFIG.ANTI_LAG_ENABLED then task.spawn(optimize,v) end
    end)
end
...
local function _main()
    ...
    local function createSettingsTab(parent)
        ...
        local floatingPanelsToggle=createToggle(uiContent,"Floating Panels",CONFIG.FLOATING_PANELS_ENABLED,function(state)
            ...
            local panels={
                ...
                _instantResetPanelGui,antiBatPanelGui,tauntPanelGui,
            }
        ...
        local lock2SpeedInput=createNumberInput(cvContent,"Lock Target 2 Speed",CONFIG.LOCK_TARGET2_CHASE_SPEED,1,999,function(value)
            CONFIG.LOCK_TARGET2_CHASE_SPEED=value
        end)
        lock2SpeedInput.LayoutOrder=51
        local meleeAimbotRangeInput=createNumberInput(cvContent,"Melee Aimbot Range",CONFIG.MELEE_AIMBOT_RANGE,1,200,function(value)
            CONFIG.MELEE_AIMBOT_RANGE=value
        end)
        meleeAimbotRangeInput.LayoutOrder=52
        local mirrorTpDownThreshInput=createNumberInput(cvContent,"Mirror Tp Down Threshold",CONFIG.MIRROR_TP_DOWN_THRESHOLD,3,100,function(value)
        ...
        local igDistInput=createNumberInput(tvContent,"Full Steal Range",CONFIG.AUTO_STEAL_ACTIVATION_DIST,0.1,9999,function(value)
            CONFIG.AUTO_STEAL_ACTIVATION_DIST=value
        end,true)
        igDistInput.LayoutOrder=2
        local igScanInput=createNumberInput(tvContent,"Stealing Range",CONFIG.AUTO_STEAL_SCAN_RANGE,1,9999,function(value)
            CONFIG.AUTO_STEAL_SCAN_RANGE=value
        end,true)
        igScanInput.LayoutOrder=3
        ...
        local stealDurInput=createNumberInput(tvContent,"Hold Duration (0=1.3s)",CONFIG.AUTO_STEAL_DURATION-1.3,0,1.3,function(value)
    ...
    local function createDuelTab(parent)
        ...
        autoSwingToggle.LayoutOrder=10
        ...
        batSpeedNormalToggle.LayoutOrder=11
        ...
        lockTarget2PanelToggle.LayoutOrder=51
        ...
        meleeAimbotPanelToggle.LayoutOrder=52
        ...
        autoUpGuiToggle.LayoutOrder=75
    ...
    local function createStatsIsland()
        ...
        pbFrame.Size=UDim2.new(0,210,0,20)
        ...
        pbStatus.Size=UDim2.new(0,60,1,0)
        ...
        pbTrack.Size=UDim2.new(1,-105,0,8)
        pbTrack.Position=UDim2.new(0,70,0.5,-4)
        ...
        distBadgeBtn.Text=string.format("%.1f",CONFIG.AUTO_STEAL_ACTIVATION_DIST)
        ...
        distBox.Text=string.format("%.1f",CONFIG.AUTO_STEAL_ACTIVATION_DIST)
        ...
        local function commitDist()
            ...
            if v and v>0 then
                ...
                distBadgeBtn.Text=string.format("%.1f",v)
            else
                distBadgeBtn.Text=string.format("%.1f",CONFIG.AUTO_STEAL_ACTIVATION_DIST)
            end
            distBox.Visible=false
        ...
        task.spawn(function()
            ...
            while pbFrame and pbFrame.Parent do
                ...
                if cur~=last then
                    last=cur
                    distBadgeBtn.Text=string.format("%.1f",cur)
                end
            end
        end)
        local lastTime=tick()
