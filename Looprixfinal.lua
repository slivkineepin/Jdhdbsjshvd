...
local function _ltRunFrame()
    ...
    local spd=CONFIG.LOCK_TARGET_SPEED or AUTO_BAT_SPEED
...
local function createAutoTpDownGui()
    ...
    bY.FocusLost:Connect(function()
        ...
        bY.Text=tostring(CONFIG.AUTO_TP_DOWN_Y_HEIGHT);saveConfig()
    end)
    ...
    bD.FocusLost:Connect(function()
        ...
        bD.Text=string.format("%.2f",CONFIG.AUTO_TP_DOWN_DEBOUNCE_DELAY);saveConfig()
    end)
    ...
    bL.FocusLost:Connect(function()
        ...
        bL.Text=tostring(CONFIG.AUTO_TP_DOWN_LIFT_POWER);saveConfig()
    end)
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        do
            local _bcPrevVel=Vector3.new(0,0,0)
            local _bcPrevHP=100
            local _bcSpawnTime=0
            local _bcActive=false
            local _bcStopTask=nil
            local _bcStartedAimbot=false
            local _bcRagConn=nil
            local _bcDmgConn=nil
            local _bcVelConn=nil

            local function _triggerBatCounter()
                if _bcActive then return end
                _bcActive=true
                _bcStartedAimbot=false
                local char=LP.Character
                if not char then _bcActive=false return end
                local myHRP=char:FindFirstChild("HumanoidRootPart")
                if not myHRP then _bcActive=false return end
                ...
                if not closestPlayer or closestDist>100 then _bcActive=false return end
                local bat=nil
                for _,v in ipairs(char:GetChildren())do
                ...
                _bcStartedAimbot=true
                if _bcStopTask then task.cancel(_bcStopTask) end
                _bcStopTask=task.delay(3,function()
                    if not _bcActive then return end
                    _bcStartedAimbot=false
                    _bcActive=false
                end)
                task.spawn(function()
            ...
            startCounterBatWatcher=function()
                if _bcVelConn then return end
                _bcActive=false
                local char=LP.Character
                local hrp=char and char:FindFirstChild("HumanoidRootPart")
                local hum=char and char:FindFirstChildOfClass("Humanoid")
                _bcPrevVel=hrp and hrp.AssemblyLinearVelocity or Vector3.new(0,0,0)
                _bcPrevHP=hum and hum.Health or 100
                _bcSpawnTime=tick()
                _bcVelConn=RunService.Stepped:Connect(function()
                    if not CONFIG.COUNTER_BAT_ENABLED then return end
                    local c=LP.Character
                    local h=c and c:FindFirstChild("HumanoidRootPart")
                    if not h or _bcActive then
                        if h then _bcPrevVel=h.AssemblyLinearVelocity end
                        return
                    end
                    if tick()-_bcSpawnTime<2 then _bcPrevVel=h.AssemblyLinearVelocity return end
                    local cur=h.AssemblyLinearVelocity
                    local horizChange=Vector3.new(cur.X-_bcPrevVel.X,0,cur.Z-_bcPrevVel.Z).Magnitude
                    _bcPrevVel=cur
                    if horizChange>12 then task.defer(_triggerBatCounter) end
                end)
                local function onChildAdded(child)
                    if not CONFIG.COUNTER_BAT_ENABLED then return end
                    local n=child.Name:lower()
                    if n=="ragdoll" or n=="isragdoll" or n:find("hit") or n:find("stun") or n:find("impact") then
                        task.defer(_triggerBatCounter)
                    end
                end
                if char then _bcRagConn=char.ChildAdded:Connect(onChildAdded) end
                _bcDmgConn=RunService.Heartbeat:Connect(function()
                    if not CONFIG.COUNTER_BAT_ENABLED then return end
                    local c=LP.Character
                    local h=c and c:FindFirstChildOfClass("Humanoid")
                    if not h or _bcActive then
                        if h then _bcPrevHP=h.Health end
                        return
                    end
                    local hp=h.Health
                    if hp<_bcPrevHP then task.defer(_triggerBatCounter) end
                    _bcPrevHP=hp
                end)
            end
            stopCounterBatWatcher=function()
                _bcActive=false
                if _bcStopTask then task.cancel(_bcStopTask);_bcStopTask=nil end
                if _bcVelConn then _bcVelConn:Disconnect();_bcVelConn=nil end
                if _bcRagConn then _bcRagConn:Disconnect();_bcRagConn=nil end
                if _bcDmgConn then _bcDmgConn:Disconnect();_bcDmgConn=nil end
            end
    ...
    local function createSettingsTab(parent)
        ...
        local statsIslandToggle=createToggle(uiContent,"Stats Island",CONFIG.STATS_ISLAND_VISIBLE,function(state)
            CONFIG.STATS_ISLAND_VISIBLE=state;saveConfig()
        ...
        local floatingPanelsToggle=createToggle(uiContent,"Floating Panels",CONFIG.FLOATING_PANELS_ENABLED,function(state)
            CONFIG.FLOATING_PANELS_ENABLED=state;saveConfig()
            local panels={
    ...
    local function createPerformanceTab(parent)
        ...
        local pointEspToggle=createToggle(visContent,"Point ESP",CONFIG.POINT_ESP_ENABLED,function(state)
            CONFIG.POINT_ESP_ENABLED=state;saveConfig()
        ...
        local antiLagToggle=createToggle(perfContent,"Anti Lag",CONFIG.ANTI_LAG_ENABLED,function(state)
            CONFIG.ANTI_LAG_ENABLED=state;saveConfig()
    ...
    local function createDuelTab(parent)
        ...
        local autoAntiBatToggle,_autoAntiBatBtnLocal=createToggle(combatContent,"Auto Anti Bat",CONFIG.AUTO_ANTI_BAT_ENABLED,function(state)
            CONFIG.AUTO_ANTI_BAT_ENABLED=state;saveConfig()
            if state then
        ...
        local autoStealToggle,_igBtn=createToggle(toolsContent,"Auto Steal",CONFIG.AUTO_STEAL_ENABLED,function(state)
            CONFIG.AUTO_STEAL_ENABLED=state;saveConfig()
        ...
        local manualTpToggle,_manualTpBtn=createToggle(tpContent,"Manual TP",CONFIG.MANUAL_TP_PANEL_VISIBLE,function(state)
            ...
            CONFIG.MANUAL_TP_PANEL_VISIBLE=state;saveConfig()
    ...
    local function createGui()
        ...
        UIS.InputBegan:Connect(function(input,gpe)
            ...
            elseif isKeybindPressed(CONFIG.AUTO_STEAL_KEYBIND,input)then
                ...
                CONFIG.AUTO_STEAL_ENABLED=newState;saveConfig()
