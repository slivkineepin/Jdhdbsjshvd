...
function runDropBrainrot()
    ...
    if CONFIG.LOCK_TARGET_ENABLED then
        ...
        lockTargetEnabled=false
        stopLockTarget()
        pcall(function()if lockTargetPanelBtn then lockTargetPanelBtn.Text="LOCK: OFF"end end)
    end
    local _wasAntiBat=CONFIG.ANTI_BAT_ENABLED
    if _wasAntiBat then _setAntiBatState(false)end
    dropBrainrotActive=true
    if dropBrainrotPanelBtn then dropBrainrotPanelBtn.Text="DROP: ON"end
    task.spawn(function()

        local _wasAntiDie=antiDieEnabled
        if not _wasAntiDie then activateAntiDie() end
        local _char=LP.Character
        local _hum=_char and _char:FindFirstChildOfClass("Humanoid")
        local _humDropConn=nil
        if _hum then
            pcall(function()_hum:SetStateEnabled(Enum.HumanoidStateType.Dead,false)end)
            pcall(function()_hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown,false)end)
            _humDropConn=_hum:GetPropertyChangedSignal("Health"):Connect(function()
                if _hum.Health<=0 then
                    pcall(function()_hum.Health=_hum.MaxHealth end)
                end
            end)
        end
        ...
        if _humDropConn then pcall(function()_humDropConn:Disconnect()end)end
        if _hum and _hum.Parent then
        ...
        if not _wasAntiDie then deactivateAntiDie() end
        if _wasAntiBat then _setAntiBatState(true)end
...
local function getClosest(pos)
    ...
    for _,plr in pairs(Players:GetPlayers()) do
        if plr~=LP and isValid(plr.Character) then
...
function startLockTarget()
    ...
    _ltCharAddedConn=LP.CharacterAdded:Connect(function(ch)
        ...
        lockTargetConnection=RunService.RenderStepped:Connect(function()
            ...
            if _ltLockedTarget and not isValid(_ltLockedTarget) then _ltLockedTarget=nil end
            if not _ltLockedTarget then _ltLockedTarget=getClosest(currentH.Position) end
            local targetChar=_ltLockedTarget
...
local function createAutoTpDownGui()
    ...
    statusBtn.MouseButton1Click:Connect(function()
        ...
        saveConfig()
        if newState and CONFIG.AUTO_TP_DOWN_MODE=="hold" then
            if getgenv()._atdStartHoldMode then getgenv()._atdStartHoldMode() end
...
local function _main()
    ...
    local function setupFloatingPanels()
        ...
        do
            ...
            local _cbGen=0
            local _cbLockConn=nil
            local _cbLockTarget=nil

            local function _cbIsValid(char)
                if not char then return false end
                local hum=char:FindFirstChildOfClass("Humanoid")
                local hrp=char:FindFirstChild("HumanoidRootPart")
                return hum and hrp and hum.Health>0
            ...
            local function _cbGetClosest(pos)
                local closest=nil
                local dist=math.huge
                for _,p in pairs(Players:GetPlayers())do
                    if p~=LP and _cbIsValid(p.Character)then
            ...
            local function _cbStopLock()
                if _cbLockConn then
                    _cbLockConn:Disconnect()
                    _cbLockConn=nil
                end
                _cbLockTarget=nil
                pcall(function()
            ...
            local function _cbStartLock()
                if _cbLockConn and _cbLockConn.Connected then return end
                _cbLockConn=RunService.RenderStepped:Connect(function()
                    if not CONFIG.COUNTER_BAT_ENABLED then _cbStopLock() return end
                    local char=LP.Character
                    if not _cbIsValid(char) then return end
                    local hrp=char.HumanoidRootPart
                    local hum=char:FindFirstChildOfClass("Humanoid")
                    hum.AutoRotate=false
                    if _cbLockTarget and not _cbIsValid(_cbLockTarget) then
                        _cbLockTarget=nil
                    end
                    if not _cbLockTarget then
                        _cbLockTarget=_cbGetClosest(hrp.Position)
                    end
                    if not _cbLockTarget then
                        hrp.Velocity=Vector3.zero
                        return
                    end
                    local targetHRP=_cbLockTarget.HumanoidRootPart
                    local predicted=targetHRP.Position+(targetHRP.Velocity*0.12)
                    local standPos=predicted+Vector3.new(0,0.5,0)
                    local moveDir=standPos-hrp.Position
                    if moveDir.Magnitude>1 then
                        hrp.Velocity=moveDir.Unit*(CONFIG.LOCK_TARGET_SPEED or 55)
                    else
                        hrp.Velocity=Vector3.zero
                    end
                ...
                _counterBatConn=RunService.Heartbeat:Connect(function()
                    ...
                    if isRagdoll then
                        if not _cbTriggered then
                            ...
                            local isTP=false
                            if hrp and _cbPrevPos then
                                if(_cbPrevPos-hrp.Position).Magnitude>50 then
                                    isTP=true
                                end
                            end
                            if not isMedusa and not isTP then
                    ...
                    if hrp then _cbPrevPos=hrp.Position end
                end)
            end
    ...
    local function createPerformanceTab(parent)
        ...
        local autoTpDownToggle=createToggle(perfContent,"Auto Tp Down",CONFIG.AUTO_TP_DOWN_PANEL_VISIBLE,function(state)
            CONFIG.AUTO_TP_DOWN_PANEL_VISIBLE=state;saveConfig()
            if _atdGuiInstance then _atdGuiInstance.Enabled=state end
        end)
    ...
    local function createDuelTab(parent)
        ...
        local autoUpGuiToggle,_auGuiBtn=createToggle(toolsContent,"Auto Up",CONFIG.AUTO_UP_PANEL_VISIBLE,function(state)
            CONFIG.AUTO_UP_PANEL_VISIBLE=state;saveConfig()
            if auGuiInstance then auGuiInstance.Enabled=state end
            if not state and AU.enabled then
                auDisable()
            end
        end)
